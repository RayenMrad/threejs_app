...
// Hit canvas for touch/tap interaction (sits on top)
const hitCanvas = document.createElement("canvas");
hitCanvas.style.cssText =
...
// Camera simulates user standing ~1.2m above floor, looking slightly down/forward
...
style.textContent = `
  ...
  *{box-sizing:border-box;-webkit-tap-highlight-color:transparent;touch-action:manipulation;}
  ...
  /* ── Reticle ── */
  #reticle-overlay{position:fixed;z-index:3;pointer-events:none;transform:translate(-50%,-50%);transition:opacity .3s;opacity:0;}
  #reticle-overlay.visible{opacity:1;}
  #reticle-overlay svg{filter:drop-shadow(0 0 6px rgba(24,83,79,.5));}
...
// ─── Reticle overlay (replaces WebXR hit-test ring) ──────────
const reticleOverlay = document.createElement("div");
reticleOverlay.id = "reticle-overlay";
reticleOverlay.innerHTML = `
  <svg width="60" height="60" viewBox="0 0 60 60" fill="none">
    <ellipse cx="30" cy="30" rx="26" ry="10" stroke="rgba(24,83,79,.9)" stroke-width="1.5" stroke-dasharray="5 4"/>
    <line x1="30" y1="18" x2="30" y2="24" stroke="rgba(24,83,79,.9)" stroke-width="2" stroke-linecap="round"/>
    <line x1="30" y1="36" x2="30" y2="42" stroke="rgba(24,83,79,.9)" stroke-width="2" stroke-linecap="round"/>
    <circle cx="30" cy="30" r="2.5" fill="#18534F"/>
  </svg>`;
document.body.appendChild(reticleOverlay);
...
// ─── Raycaster (replaces WebXR hit-test) ─────────────────────
const raycaster = new THREE.Raycaster();
// Current reticle position in screen coords (updated on touch/mouse move)
let reticleScreenX = window.innerWidth / 2;
let reticleScreenY = window.innerHeight / 2;
const floorPos = new THREE.Vector3();
let floorFound = false;

function updateFloorFromScreen(sx, sy) {
...
function startPreview() {
  ...
  setPlaceIcon("aim");
  setScan(true);
  topStatus.textContent = "Tap on floor to place";
  setHint(null);
  moveBanner.style.display = "none";
  updateRotUI();
  if (panelCollapsed) {
...
function doConfirm() {
  if (appMode === "previewing" && previewObj && floorFound) {
    ...
    topStatus.textContent = "Placed!";
    setHint("Tap ↺ to undo · hold ⟳ to rotate · tap ✦ to move");
    reticleOverlay.classList.remove("visible");
    updateRotUI();
    setTimeout(() => {
      setPlaceIcon("aim");
      setHint(null);
...
// ─── Touch / tap interaction (replaces XR select event) ──────
let dragStartX = 0,
  dragStartY = 0,
  isDragging = false,
  hasMoved = false;
let isDomTouch = false;

// Track if touch started on UI elements
document.getElementById("ui-bottom").addEventListener(
  "pointerdown",
  () => {
    isDomTouch = true;
...
hitCanvas.addEventListener(
  ...
  (e) => {
    ...
    isDragging = true;
    hasMoved = false;
    updateReticlePos(t.clientX, t.clientY);
  },
  { passive: true },
);
...
hitCanvas.addEventListener(
  ...
  (e) => {
    ...
    // Drag to reposition selected/previewing object
    if (hasMoved && (appMode === "selected" || appMode === "previewing")) {
      const target = appMode === "previewing" ? previewObj : selectedObj;
      if (target) {
        updateFloorFromScreen(t.clientX, t.clientY);
        if (floorFound) {
          positionObject(target);
          if (appMode === "selected") {
            // update frozen so it sticks when confirmed
            frozenRotations.set(target, target.rotation.y);
          }
        }
      }
    }
...
hitCanvas.addEventListener("touchend", (e) => {
  ...
  isDragging = false;
  if (isDomTouch) return;
  const t = e.changedTouches[0];
  if (!hasMoved) handleTap(t.clientX, t.clientY);
});
...
hitCanvas.addEventListener("mousedown", (e) => {
  ...
  isDragging = true;
  hasMoved = false;
  updateReticlePos(e.clientX, e.clientY);
});
hitCanvas.addEventListener("mousemove", (e) => {
  ...
  if (hasMoved && (appMode === "selected" || appMode === "previewing")) {
    const target = appMode === "previewing" ? previewObj : selectedObj;
    if (target) {
      updateFloorFromScreen(e.clientX, e.clientY);
      if (floorFound) positionObject(target);
    }
  }
});
hitCanvas.addEventListener("mouseup", (e) => {
  if (!isDragging) return;
  isDragging = false;
  if (isDomTouch) return;
  if (!hasMoved) handleTap(e.clientX, e.clientY);
});

function updateReticlePos(sx, sy) {
  reticleScreenX = sx;
  reticleScreenY = sy;
  reticleOverlay.style.left = sx + "px";
  reticleOverlay.style.top = sy + "px";
  updateFloorFromScreen(sx, sy);
  if (appMode === "previewing" && previewObj && floorFound) {
    previewObj.visible = true;
  ...
  if (appMode === "selected" && selectedObj && floorFound) {
    positionObject(selectedObj);
  }
}

function handleTap(sx, sy) {
  // Tap on existing placed object → select it
  if (appMode === "idle" && placedList.length > 0) {
    const hit = screenToObject(sx, sy);
    if (hit) {
      selectedObj = hit;
      appMode = "selected";
      moveBanner.style.display = "block";
      setPlaceIcon("move");
      topStatus.textContent = "Moving item";
      reticleOverlay.classList.add("visible");
      reticleOverlay.style.left = sx + "px";
      reticleOverlay.style.top = sy + "px";
      updateRotUI();
      return;
...
aDelOne.addEventListener("click", (e) => {
  ...
  if (appMode === "selected" && selectedObj) {
    ...
    setHint(null);
    reticleOverlay.classList.remove("visible");
    topStatus.textContent = "Item deleted";
    updateRotUI();
    setTimeout(() => (topStatus.textContent = "Ready"), 1200);
  }
});
...
renderer.setAnimationLoop(() => {
  // Keep placed objects at their frozen rotation unless being moved
  placedList.forEach((obj) => {
    if (obj !== selectedObj) {
      obj.rotation.y = frozenRotations.get(obj) ?? 0;
    }
